...
if (rank == 0) {
  ...
  source = dest;
  /*
    SOLUTION: fixed *tag* to be the same value for each process 
    so messages sent with a tag will be recieved by a process looking
    for a message with the same tag. 
  */
  tag = numtasks;
